...
import type { Document, Filter, FindOptions, Sort, UpdateFilter } from "mongodb";
...
export class MongoDBDataAPI<InnerDoc = Document> {
    ...
    public $cluster(clusterName: string) {
        return this.#newAPI<InnerDoc>({ dataSource: clusterName }) as Omit<MongoDBDataAPI<InnerDoc>, "$cluster" | "$collection">;
    }

    /** Select a database. */
    public $database(database: string) {
        return this.#newAPI<InnerDoc>({ database }) as Omit<MongoDBDataAPI<InnerDoc>, "$cluster" | "$database">;
    }

    /** Select a collection. */
    public $collection<Doc = InnerDoc>(collection: string) {
        return this.#newAPI<Doc>({ collection }) as Omit<MongoDBDataAPI<Doc>, "$cluster" | "$database" | "$collection">;
    }
     ...
     */
    public $$action<Result = unknown>(name: string, params: BaseParams = {}, axiosConfig?: AxiosRequestConfig): Promise<Result> {
        const mergedParams = {
            ...this.#baseParams,
            ...params,
        };
     ...
     */
    public insertOne<D = InnerDoc, T = NoInfer<D>>(params: ExtendBaseParams<{ document: AnyKeys<T> | Document }>) {
     ...
     */
    public insertMany<D = InnerDoc, T = NoInfer<D>>(params: ExtendBaseParams<{ documents: Array<AnyKeys<T> | Document> }>) {
     ...
     */
    public deleteOne<D = InnerDoc, T = NoInfer<D>>(params: ExtendBaseParams<{ filter: Filter<T> }>) {
     ...
     */
    public deleteMany<D = InnerDoc, T = NoInfer<D>>(params: ExtendBaseParams<{ filter: Filter<T> }>) {
     ...
     */
    public aggregate<T extends Array<any>>(params: ExtendBaseParams<{ pipeline: Array<Document> }>) {
